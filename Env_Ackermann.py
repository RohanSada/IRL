...
class AckermannVehicleEnv(gym.Env):
    ...
    def __init__(self,
                 ...
                 num_obstacles=20,
                 obstacle_radius=1.0,
                 world_bounds=(0.0, 100.0, 0.0, 100.0),
                 num_lidar_beams=10,
        ...
        self.action_space = spaces.Box(low=np.array([-2.0, -np.radians(30)]), 
                                       high=np.array([2.0, np.radians(30)]),
                                       dtype=np.float32)
        
        self.state_dim = 6 + self.num_lidar_beams
    ...
    def _generate_obstacles(self):
        ...
        for i in range(self.num_obstacles):
            while True:
                ...
                if np.linalg.norm(point - np.array(self.start_pos)) < 10.0 or np.linalg.norm(point - self.goal_pos) < 10.0:
                    continue
    ...
    def _simulate_lidar(self):
        ...
        angle_span = math.radians(self.lidar_span)
        start_angle = self.vehicle_heading - angle_span/2.0
